    ...
    while True:
        ...
        for inde, i in enumerate(I):
            ...
            if delta_i == 0:
                ...
                vi[iteration].append(vi[iteration - 1][i - 1])
                sj.append(sj[-1])
            ...
            delta = min(delta_i,
                        sortedCostMatrix[i - 1][k[i - 1] - 1] - vi[- 2][i - 1]
                        )
            ...
            ji_neu: int = costMatrix[i - 1].index(sortedCostMatrix[i - 1][k[i - 1] - 1])
            k[i - 1] += 1
            vi[- 1].append(vi[- 2][i - 1] + delta)
